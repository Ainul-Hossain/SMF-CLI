...
import { FLEX,GAP, HEIGHT, WIDTH, COLOR, FONTSIZE, FONTWEIGHT } from '../Theme/theme'
...
const {EditMusic,setEditMusic,  loadedSounds, setLoadedSounds, rain_sound, set_rain_sound,
...
const Play_Audio = async (item) => {
    try {
      ...
      if (
        playingSounds.some(
          (playingSound) => playingSound._uri === item.url
        )
      ){
...
const handle_edit_music = (item) => {
    ...
    if (
      !EditMusic.some((existingItem) => areObjectsEqual(existingItem, item))
    ) {
        ...
        >
          ...
          <View style={styles.sound_btn_container}>
            <TouchableOpacity
              ...
              style={[styles.resetBtn, {opacity: EditMusic.length === 0 ? 0.48 : null}]}
            ...
            >
              <LinearGradient
                ...
                style={[styles.Apply_Box_linearGradient, {opacity:EditMusic.length === 0 ? 0.48 : null }]}
                colors={TrailColor}
              >
